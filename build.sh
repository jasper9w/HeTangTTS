...
rm -rf dist/hetangtts dist/hetangtts.app
uv run pyinstaller hetangtts.spec --clean
if [ $? -ne 0 ]; then
    echo -e "${RED}打包失败${NC}"
    exit 1
fi
...
echo -e "\n${GREEN}=== 构建成功 ===${NC}"
echo -e "${GREEN}应用位置: ${YELLOW}dist/hetangtts.app${NC}"
echo -e "${BLUE}提示: 可以将 hetangtts.app 拖到应用程序文件夹${NC}"
